...
app.layout = dbc.Container([
    html.H1("Visualisations Dashboard", style = {'background': '#2b5dba', 'color': 'white', 'margin-bottom': 0},), 
    html.Div(
            ...
            dbc.Col(
                width=4, children=[
                    ...
                    dcc.Textarea(
                        ...
                        style={'width': '100%', 'height': 125, 'resize': 'none'},
                        maxLength=300,
                    ),
                    html.Button(
                        'Submit',
            ...
            dbc.Col(
                width=4, children=[
                    ...
                    html.Button(
                        ...
                        id = 'ratings_submit',
                        n_clicks=0,
                        style = {'background-color': 'DeepSkyBlue', 'float': 'right'}
                    )
                ]
            ),
        ]
...
@app.callback(Output("bar_HG", "figure"), Input("season_select", "value"))
def update_bar_chart(season_select):
    bar_goal_default = bar_goals_list[season_select]
    return bar_goal_default


@app.callback(Output("stats_card", "children"), Input("season_select", "value"))
...
@app.callback(Output("line_HG", "figure"), Input("team_select", "value"))
...
@app.callback(Output("team_stats_card", "children"), Input("team_select", "value"))
def render_stats_panel(team_select):
    ...
    old_average_goals = (total_goals[team_select].sum() - present_goals) / 4
